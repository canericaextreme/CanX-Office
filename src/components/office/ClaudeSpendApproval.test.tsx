...
const h=vi.hoisted(()=>({request:vi.fn(),remember:vi.fn(),open:vi.fn(),verified:true,states:[] as unknown[]}));
...
vi.mock("@/lib/owner-session",()=>({useOwnerSession:()=>({stepUpComplete:h.verified,accessToken:"owner-token"})}));
...
beforeEach(()=>{h.request.mockReset();h.remember.mockReset();h.open.mockReset();h.states=[];h.verified=true;vi.stubGlobal("window",{dispatchEvent:vi.fn()});});
describe("one owner-approved Claude review",()=>{
 it("does not call Claude on render; a double click sends only one office review",async()=>{
  ...
  expect(h.request).toHaveBeenCalledOnce();expect(h.request.mock.calls[0]![0].data).toMatchObject({scope:"office",accessToken:"owner-token"});
